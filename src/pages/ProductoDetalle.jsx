...
export default function AdminProductos() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (imageFile) {
      try {
        ...
        try { uploadData = JSON.parse(text); }
          ...
          setError('Error del servidor al subir imagen. Intenta con una imagen más pequeña (menos de 3MB).');
          setLoading(false);
          return;
        }
        if (!uploadRes.ok) {
          setError('Error al subir imagen: ' + (uploadData?.error || uploadData?.details || uploadRes.status));
          setLoading(false);
          return;
        }
        finalImageUrl = uploadData.url;
      } catch (err) {
        setError('Error de red al subir imagen. Verifica tu conexión.');
        setLoading(false);
        return;
      }
    }
    ...
    if (!res.ok) {
      ...
      setError('Error al guardar producto: ' + (err.details || err.error || res.status));
      setLoading(false);
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          <div className="lg:col-span-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="flex flex-col gap-2">
              ...
              <select value={form.stock_status} onChange={e => setForm({...form, stock_status: e.target.value})} className="px-4 py-3 bg-gray-50 border border-gray-200 rounded-lg focus:outline-none focus:ring-1 focus:ring-black focus:border-black transition-all text-sm text-gray-800">
            ...
            <div className="flex flex-col gap-2 md:col-span-2">
              <label className="text-xs font-bold uppercase text-gray-500 tracking-wider flex justify-between">
                ...
                <span className="text-gray-400 font-normal lowercase tracking-normal">Separa cada punto con Enter</span>
              </label>
          ...
          <div className="lg:col-span-4 flex flex-col gap-2">
            <label className="text-xs font-bold uppercase text-gray-500 tracking-wider">
              Fotografía
              <span className="ml-1 text-gray-400 font-normal lowercase tracking-normal normal-case text-[10px]">(se comprime automáticamente)</span>
            </label>
                ...
                <tr key={product.id} className="hover:bg-gray-50/50 transition-colors group">
                  <td className="px-6 py-4">
                    <div className="h-14 w-14 rounded-lg bg-gray-50 border border-gray-100 p-1 flex items-center justify-center overflow-hidden">
                      {product.image_url && <img src={product.image_url} alt={product.name} className="max-h-full object-contain" />}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => handleEdit(product)} disabled={loading} className="text-gray-400 hover:text-black text-xs font-bold uppercase tracking-widest transition-colors mr-5">Editar</button>
                    <button onClick={() => handleDelete(product)} disabled={loading} className="text-gray-400 hover:text-red-500 text-xs font-bold uppercase tracking-widest transition-colors">Eliminar</button>
                  </td>
                </tr>
              ))}
            </tbody>
